...
import type { JobParams, Platform } from '../types'
...
const PLATFORMS: { value: Platform; label: string }[] = [
  ...
  { value: 'td_direct',   label: 'TD Direct Investing' },
]
...
export default function ConfigForm({ onJobStarted, disabled }: Props) {
  ...
  const [platform, setPlatform]       = useState<Platform>('questrade')
  ...
  useEffect(() => {
    getSectors().then(setAllSectors).catch(() => setAllSectors([]))
  }, [])
            ...
            {allSectors.map(s => {
              ...
              return (
                <button
                  ...
                  style={active
                    ? { borderColor: BRAND, color: BRAND, background: BRAND_LIGHT, fontWeight: 600 }
